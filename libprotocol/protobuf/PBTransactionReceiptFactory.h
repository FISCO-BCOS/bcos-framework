...
public:
    ...
    TransactionReceipt::Ptr createReceipt(int32_t _version, h256 const& _stateRoot,
        u256 const& _gasUsed, Address const& _contractAddress, LogEntriesPtr _logEntries,
        int32_t _status, bytes const& _output) override
    {
        return std::make_shared<PBTransactionReceipt>(m_cryptoSuite, _version, _stateRoot, _gasUsed,
            _contractAddress, _logEntries, _status, _output);
    }

    TransactionReceipt::Ptr createReceipt(int32_t _version, h256 const& _stateRoot,
        u256 const& _gasUsed, Address const& _contractAddress, LogEntriesPtr _logEntries,
        int32_t _status, bytes&& _output) override
    {
        return std::make_shared<PBTransactionReceipt>(m_cryptoSuite, _version, _stateRoot, _gasUsed,
            _contractAddress, _logEntries, _status, _output);
    }
